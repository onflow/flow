...
By allowing users to annotate their functions as `pure`, we can enforce that these functions have no side effects that modify state
...
This adds a new keyword to the langauge: `pure`. This is optionally placed before the `fun` keyword, but after the access modifier, 
...
```cadence
    ...
    let b: ((): Void) = pure fun() {}
    let c: ((): Void) = fun() {}
    let d: (((pure (): Void)): Void) = fun foo(x:((): Void)) {} // contravariance
```
...
to use the new `pure` annotations. However, these annotations are unlikely to be frequently required; they will only be necessary for functions that are 
...
`pure` annotations to those functions. However, if users have other reasons for wanting to limit what mutations can be performed from inside functions they
may choose to add `pure` annotations anyways. 

### User Impact

This will have little direct impact on users, as `pure` annotations will only be required on functions that are called from conditions. This is, however, 
