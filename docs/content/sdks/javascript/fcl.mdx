...
A high level abstraction (built on top of [@onflow/sdk](/sdks/javascript)) that enables development of browser based dApps.
...
Cadence scripts allow you to run computations on Flow that are not recorded on the blockchain. You'll mainly use scripts as a way of querying Flow for information about accounts and their **[resources](/tutorial/cadence/03-fungible-tokens/#intuiting-ownership-with-resources).** A script is not signed by any account and cannot modify an account's state.
...
A common use for Cadence scripts is to acquire information about a Flow account's public state and its **[resources](/tutorial/cadence/03-fungible-tokens/#intuiting-ownership-with-resources)**. Here is an example of a script you might use to query and use another account's public resources.
