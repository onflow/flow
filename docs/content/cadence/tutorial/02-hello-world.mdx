...
- The `private` domain is like a private API. You can optionally store [capabilities](/intro/glossary/#capability) to any of your stored assets here. Only the owner and anyone they give access to can use these interfaces to call functions that are defined in your private assets. Users will commonly store private capabilities here that refer to other account's private objects.
- The `public` domain is kind of like your account's public API. The owner can store capabilities here that anyone else in the network can access to interact with the private assets that are stored in the account.
...
In this case, this is the first transaction we have run, so we know that the storage spot at `/storage/Hello is empty. In real applications, we would likely perform necessary checks and actions with the location we are storing to make sure we don't abort the transaction because of an accidental overwrite.
