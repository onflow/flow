...
Storage capacity of an account is dictated by the amount of FLOW tokens it has. **The minimum amount of FLOW tokens an account can have is 0.001. This minimum is provided by the account creator at account creation.** The minimum account reservation ensures that most accounts won't run out of storage capacity if anyone deposits anything (like an NFT) to the account.

The exact amount of storage capacity an account has is the amount of FLOW tokens the account has times the `storageBytesPerReservedFlow` variable defined on the `StorageFees` smart contract, which is 10 MiB per 1 reserved FLOW token.
...
- Minimum account FLOW is **0.001 Flow tokens** and represents **10 KiB** of storage capacity. This is also the amount of Flow tokens that the creator of a new account needs to provide for the account's storage reservation. (`StorageFees.minimumStorageReservation`)
- The amount of storage capacity one Flow token represents is **10 MiB per Flow token**. (`StorageFees.storageMegaBytesPerReservedFlow`)
