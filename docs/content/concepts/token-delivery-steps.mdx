...
If you invested through Coinlist, it is the default option, meaning your tokens will be deposited there if you do not take any action. To receive your tokens through another custodian or wallet provider, you must create a Flow address by following the steps outlined below. You only need **one** Flow address so you only go through the process with a single provider. 
