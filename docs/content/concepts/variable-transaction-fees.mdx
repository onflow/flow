...
## Understanding need for transaction fees
...
In the future, costs for inclusion will be impacted by the byte size of transaction and the number of signatures required.
...
Storage fees are implemented differently to transaction fees. Read the [Storing Data on Flow guide](https://docs.onflow.org/concepts/storage/#storage-capacity) for more details. In summary, storage fees are a cost associated with storing data on chain.

## Estimating transaction costs

Cost estimation is a two-step process. First, you need to gather the execution effort with either the emulator or on the testnet. Second, you use the execution effort for a transaction to calculate the final fees using one the JavaScript or Go FCL SDKs.
...
It is important to set a limit that isn’t too high or too low. If it is set too high, the payer needs to have more funds in their account before sending the transaction. If it is too low, the execution could fail and all state changes be dropped.
...
There are several optimizations that can lead to reduced execution time of transactions. Below is a list of some practices. This list is not exhaustive but rather exemplary.
...
Whenever you want to iterate over a list, make sure it is absolutely necessary to iterate through all elements as opposed to a subset. Avoid loops to grow in size too much over time. Limit loops when possible.
...
### Understand impact of function calls
...
Creating accounts and adding keys is associated with costs. Try to only create accounts and keys when necessary.
...
### Explain that waiting for network surge to pass is an option
...
### Explain that the wallet might not allow the transaction due to lack of funds
...
Final fees are determined by the surge factor on the network. The surge factor for testnet will be different to the factor on mainnet, so you need to expect a variation between mainnet and testnet estimates.
