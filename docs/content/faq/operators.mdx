...
No errors should be considered acceptable. If there are errors that are constantly recurring, please bring it to to the core team's attention so logging can be adjusted accordingly, if required.
...
See the dedicated section on node operation: [https://docs.onflow.org/node-operation/](https://docs.onflow.org/node-operation/)
