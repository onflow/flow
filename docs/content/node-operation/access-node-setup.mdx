...
Currently on mainnet, the staking auction starts every Wednesday at around 17:30 UTC and ends on the next Wednesday at 8:30 UTC.
Hence, the node should be staked by _Wednesday, 08:30 UTC_ to be able to join the network in the next epoch.
...
If there are more than five staked candidate access nodes at the end of the staking phase of the current epoch which ends on Wedensday 15th Feb, five of those will be selected by an on-chain random selection process and no other permissionless ANs can join until others unstake in a future epoch.
...
|:----------:|:---------:|:---------:|:----------------------:|:---------------------------------------------------:|
| 02/08/2023 | 17:30 UTC | n         | Staking auction starts | Stake your access nodes                             |
| 02/15/2023 | 08:00 UTC | n         | Staking auction ends   | No more access nodes can be staked for epoch n+1    |
| 02/15/2023 | 17:30 UTC | n+1       | Epoch n+1 starts       | 5 randomly chosen access nodes can join the network |

> Please ensure that you have staked the access node by **15th Feb, 2023 Wednesday 8:00 AM UTC** if you wish to run the node in the next epoch.
...
3. You can verify if your node ID was selected by the on-chain random selection process on Wednesday at around 17:30 UTC when the next epoch starts.
...
On Wednesday at 17:30 AM UTC, the staking auction for the current epoch will end and five nodes from candidate list of nodes will be chosen at random by the staking contract to be part of the next epoch.

>Note: If all 5 slots have been taken from the previous epoch, the no new access nodes will be chosen (see #limitations)
...
The limits for the open slots are defined in the staking contract and can be queried from the chain by executing the [get_candidate_limit](https://github.com/onflow/flow-core-contracts/blob/master/transactions/idTableStaking/scripts/get_candidate_limits.cdc) script.

```shell
$ flow scripts execute ./transactions/idTableStaking/scripts/get_candidate_limits.cdc -n mainnet
Result: {1: 1, 4: 1, 2: 1, 3: 1, 5: 5}
```
