...
If there are more than five staked candidate access nodes at the end of the staking phase on 14th Feb 2023 EOD PT, five of those will be selected by an on-chain random selection process and no other permissionless ANs can join until others unstake in a future epoch.
...
2. Stake the node with 100 FLOW by **EOD Tuesday, 14th February 2023 Pacific time (02/14/2023 11:59 PM or 02/15/2023 7:59 AM UTC)** (see [timing](#timing)) by providing the node information generated in step 1.
3. You can verify if your node ID was selected by the on-chain random selection process on the next day, Wednesday (15th February 2023).
...
> Note: You will need to run this step for each access node that you would like to run.
...
On Wednesday, 15th Feb 2023 at ~3:30 AM Pacific (11:30 AM UTC), the staking auction for the current epoch will end and five nodes from candidate list of nodes will be chosen at random by the staking contract to be part of the next epoch.

There are several ways to verify whether your node was chosen as explained below.

When you stake the node, the tokens will show up under the `tokensCommitted` bucket. After the staking auction ends, if the node is selected, the tokens are moved to the `tokensStaked` bucket.
