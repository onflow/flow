...
3. Check if your node ID got selected by the random on-chain selection process on Wednesday.
4. If your node ID got selected, provision and start running the node. If your node didn't get selected, your staked tokens you can stake try again in the next epoch. Your staked tokens will be returned in that case.

Following is how to execute each of these four steps in details:
...
## Step 3 - Check if the node ID got selected by the random on-chain selection process
