...
Report the suspicious node on Discord on the `#flow-validators` channel along with all the evidence (log messages, other networking related metrics etc.)
All other node operators will be informed about the suspicious node via that channel.

## Blocking a byzantine node

Once a consensus is reached about the suspicious node on Discord amond the node operators, the suspicious node can be blocked using the admin command.
