...
An observer node is similar to an access node and provides a locally accessible, continuously updated, verified copy of the block data. It serves the Access API ([gRPC](https://github.com/onflow/flow/blob/master/docs/content/access-api.md) and [REST](https://developers.flow.com/http-api/)) but unlike an access node, an observer node does not need to be staked, and **anyone** can run it without being added to the approved list of nodes.
...
1. Does the observer node need to be staked?

No, the observer node is not a staked node.

2. Can any access be used to bootstrap an observer node?
...
> While the public keys remain the same, the hostnames change each spork to include the spork name. Substitute `[devnet_spork]` and `[mainnet_spork]` with the appropriate spork name (e.g. `mainnet14`). See [Past Sporks](https://docs.onflow.org/node-operation/past-sporks/) for the current spork for each network.
