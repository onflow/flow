...
The `root-protocol-state-snapshot.json` is generated for each [spork](https://developers.flow.com/nodes/nodes/node-operation/spork) and contains the genesis data for that spork.
...
While the public keys remain the same, the hostnames change each spork to include the spork name. Substitute `[current mainnet spork]` and `[current devnet spork]` with the appropriate spork name (e.g. `mainnet19`).
See [Past Sporks](https://docs.onflow.org/nodes/node-operation/past-sporks/) for the current spork for each network.