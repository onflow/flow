...
An observer node is similar to an access node and provides a locally accessible, continuously updated, verified copy of the block data. It serves the Access API ([gRPC](https://developers.flow.com/nodes/access-api.md) and [REST](https://developers.flow.com/http-api/)) but unlike an access node, an observer node does not need to be staked, and **anyone** can run it without being added to the approved list of nodes.
