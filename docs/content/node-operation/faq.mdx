...
See [Staking & Epochs](https://developers.flow.com/nodes/staking/epoch-scripts-events) for general information and [Node Setup](node-bootstrap.mdx#timing) for a guide to running a new node.
...
To verify if a node is online, please [setup metrics](faq.mdx#how-do-i-check-on-the-status-of-my-node) for the node.
