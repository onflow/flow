...
An observer node is similar to an access node and provides a locally accessible, continuously updated and verified copy of the block data. It serves the Access API ([gRPC](https://github.com/onflow/flow/blob/master/docs/content/access-api.md) and [REST](https://developers.flow.com/http-api/)) but, unlike an access node, an observer node does not need to be staked and **anyone** can run it without being added to the approved list of nodes.
...
However, it makes no trust assumption of the upstream access node or the observer node which is providing the block and locally verifies that the blocks that are received are the correct extension of the chain e.g After receiving valid blocks A, B and C when it receives block D, it verifies that block D is indeed signed by the consensus nodes and is a valid next block. The block data that is received is indexed and made available via the Access API. For Collection, Transactions and Account queries, it delegates those requests to the upstream access node. Similarly, transactions and scripts sent to an observer node are also forwarded to the upstream access node. Future versions of the observer node will be able to serve this data locally as well.
...
For mainnet-19
```
wget -P ./flow_observer/bootstrap/public-root-information https://storage.googleapis.com/flow-genesis-bootstrap/mainnet-19-execution/public-root-information/root-protocol-state-snapshot.json
wget -P ./flow_observer/bootstrap/public-root-information https://storage.googleapis.com/flow-genesis-bootstrap/mainnet-19-execution/public-root-information/root-protocol-state-snapshot.json.asc
```

For testnet-37
```
wget -P ./flow_observer/bootstrap/public-root-information https://storage.googleapis.com/flow-genesis-bootstrap/testnet-37/public-root-information/root-protocol-state-snapshot.json
wget -P ./flow_observer/bootstrap/public-root-information https://storage.googleapis.com/flow-genesis-bootstrap/testnet-37/public-root-information/root-protocol-state-snapshot.json.asc
```
...
For mainnet
```
 flow snapshot save ./flow_observer/bootstrap/public-root-information/root-protocol-state-snapshot.json --host access-008.mainnet19.nodes.onflow.org:9001 --network-key 11742552d21ac93da37ccda09661792977e2ca548a3b26d05f22a51ae1d99b9b75c8a9b3b40b38206b38951e98e4d145f0010f8942fd82ddf0fb1d670202264a
```

For testnet
```
flow snapshot save ./flow_observer/bootstrap/public-root-information/root-protocol-state-snapshot.json --host access-003.devnet37.nodes.onflow.org:9001 --network-key b662102f4184fc1caeb2933cf87bba75cdd37758926584c0ce8a90549bb12ee0f9115111bbbb6acc2b889461208533369a91e8321eaf6bcb871a788ddd6bfbf7
```
...
docker run --rm \
 ...
 --upstream-node-addresses=access-008.mainnet19.nodes.onflow.org:9001 \
 ...
 --bootstrap-node-addresses=access-008.mainnet19.nodes.onflow.org:3570  \
...
```
curl "localhost:8071/v1/blocks?height=final"
```

### FAQs

1. Does the observer node needs to be staked?

No, the observer node is not a staked node.

2. Can any access not be used to bootstrap an observer node?
...
The public access nodes that support observer nodes are listed above. Apart from these, other public access nodes run by node operators other than DapperLabs may choose to support observer node.

3. How can an access node turn ON support for observer node?

An access node can support an observer node by passing in these two parameters
```
...
4. Are observer nodes subject to rate limits?
