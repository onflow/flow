...
The `systemd` files pull runtime setting from `/etc/flow/runtime-config.env` and any `.env`
...
Ensure you start your node at the appropriate time. See [Spork Process](/node-operation/spork) for 
when to start up a node following a spork. See [Node Bootstrap](/node-operation/node-bootstrap#timing) for
when to start up a newly registered node.
