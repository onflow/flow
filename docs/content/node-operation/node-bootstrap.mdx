...
If you are running a collection and consensus node, you will have an additional private key file
...
In Step 2 of this guied, when you submit a transaction to stake your node, you will need to provide the
machine account public key, which can be found in the output of previous `bootstrap key` command.

```shell:title=Machine Account public key in command output
