...
If you had a previous node running on **mainenet 6**, you’ll need to turn it
...
d9a43a68cb4a53728c26d1c49c7d37c9  ./boot-tools/bootstrap

md5sum ./boot-tools/transit
3edad4341e3ac2c3cb22387f698f5785  ./boot-tools/transit
```
...
<Callout type="warning">
  ...
  Please use the following pattern when generating your token `mainnet-7-yourname` (you can pick any name, it just helps us keep track of the most recent keys you've sent and would like us to include in the bootstrapping process)
...
```shell:title=Example
$ ./boot-tools/transit pull -d ./bootstrap -t mainnet-7  -role consensus
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/access:v0.15.3-patch.4 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/collection:v0.15.3-patch.4 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/consensus:v0.15.3-patch.4 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/execution:v0.15.3-patch.4 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/verification:v0.15.3-patch.4 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
