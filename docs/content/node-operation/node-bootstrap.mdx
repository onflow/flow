...
If you had a previous node running on **mainnet 12**, you’ll need to turn it
...
<Callout type="warning">
  ...
  Please use the following pattern when generating your token `mainnet-13-yourname` (you can pick any name, it just helps us keep track of the most recent keys you've sent and would like us to include in the bootstrapping process)
...
```shell:title=Example
$ ./boot-tools/transit pull -b ./bootstrap -t mainnet-13  -r consensus
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/access:v0.21.3 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/collection:v0.21.3 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--secretsdir=/data/secrets \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/consensus:v0.21.3 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--secretsdir=/data/secrets \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/execution:v0.21.3 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--secretsdir=/data/secrets \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/verification:v0.21.3 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--secretsdir=/data/secrets \
