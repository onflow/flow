...
If you had a previous node running on **mainnet 10**, you’ll need to turn it
...
<Callout type="warning">
  For `consensus` and `collection` node types a key known as the Machine account will be generated. For all other node types this will not be needed.
</Callout>

#### Generate Machine Account Address

Once you have generated all your keys, you will need to now submit a transaction on Flow Port to create a Machine Account for the Machine Account public key. In the output of 
the `boostrap key` command from the previous step you will see your Machine Account public key.
...
You can follow the guide [here](/flow-port/staking-guide) to submit a transaction on [Flow Port](https://port.onflow.org/).
...
<Callout type="warning">
  ...
  Please use the following pattern when generating your token `mainnet-11-yourname` (you can pick any name, it just helps us keep track of the most recent keys you've sent and would like us to include in the bootstrapping process)
...
```shell:title=Example
$ ./boot-tools/transit pull -b ./bootstrap -t mainnet-11  -r consensus
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/access:v0.19.2 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/collection:v0.19.2 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/consensus:v0.19.2 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/execution:v0.19.2 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/verification:v0.19.2 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
