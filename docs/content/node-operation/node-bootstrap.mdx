...
This guide is for getting a new node staked and running on Flow other than a permissionless Access node. For running a permissionless Access node see [Access node setup](https://developers.flow.com/nodes/node-operation/access-node-setup). For sporking documentation for existing node operators, see [Spork Process](/nodes/node-operation/spork).
