...
This guide is for getting a new node staked and running on Flow. For sporking documentation for existing node operators, see [Spork Process](spork.mdx).
...
```
4. Start your node (see [guide](node-setup.mdx#start-the-node))
...
A snapshot can be obtained from any Access Node using [Flow CLI](https://developer.flow.com/tools./flow-cli).
