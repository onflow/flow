...
If you had a previous node running on **mainnet 11**, you’ll need to turn it
...
d585d8d6817266f824454c5f111299e1db2f21c58c61aa30c3fcdfc6cc64fb85  ./boot-tools/bootstrap

sha256sum ./boot-tools/transit
eea7da977f8f7d598028b7dc0f372e43e62573aad3ecfdf5b19fa9117f8712cd  ./boot-tools/transit
```
...
<Callout type="warning">
  ...
  Please use the following pattern when generating your token `mainnet-12-yourname` (you can pick any name, it just helps us keep track of the most recent keys you've sent and would like us to include in the bootstrapping process)
...
```shell:title=Example
$ ./boot-tools/transit pull -b ./bootstrap -t mainnet-12  -r consensus
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/access:v0.20.5 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/collection:v0.20.5 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/consensus:v0.20.5 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/execution:v0.20.5 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/verification:v0.20.5 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
