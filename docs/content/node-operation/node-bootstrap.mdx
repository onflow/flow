...
<Callout type="warning">
  ...
  Please use the following pattern when generating your token `mainnet-4-yourname` (you can pick any name, it just helps us keep track of the most recent keys you've sent and would like us to include in the bootstrapping process)
...
```shell:title=Example
$ ./boot-tools/transit -pull -d ./bootstrap -t mainnet-4  -role consensus
...
<Callout type="warning">
Please make sure you are running Docker version 19 by running `docker --version`. There is currently an issue with Docker version 20
</Callout>
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/access:v0.13.1 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/collection:v0.13.1 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/consensus:v0.13.1 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/execution:v0.13.1 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
...
docker run --rm \
	...
	--name flow-go \
	--network host \
	gcr.io/flow-container-registry/verification:v0.13.1 \
	--nodeid=${FLOW_GO_NODE_ID} \
	--bootstrapdir=/bootstrap \
	--datadir=/data/protocol \
	--bind 0.0.0.0:3569 \
