...
<Callout type="warning">
  ...
  Please use the following pattern when generating your token `mainnet-5-yourname` (you can pick any name, it just helps us keep track of the most recent keys you've sent and would like us to include in the bootstrapping process)
...
```shell:title=Example
$ ./boot-tools/transit -pull -d ./bootstrap -t mainnet-5  -role consensus
