...
This guide is for getting a new node staked and running on Flow. For sporking documentation for existing node operators, see [Spork Process](/node-operation/spork-process).
...
```shell:title=ExampleDynamicStartupFlags
  ... \
  --dynamic-startup-access-address=access-001.mainnet16.nodes.onflow.org:9001 \
...
A snapshot can be obtained from any Access Node using [Flow CLI](https://docs.onflow.org/flow-cli/).
...
Since Collection and Consensus Nodes must start up in the first ~30mins of the `Epoch Setup Phase` (see [Timing](/node-operation/node-bootstrap/#timing)),
