...
This document outlines the steps a token holder can take to stake using [the `FlowIDTableStaking` contract](/core-contracts/flow-id-table-staking/)
...
However, if an account does an associated locked account, when the Staking Collection is initialized,
