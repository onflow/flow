...
instead. This style of staking is for those who want to keep their staking operation as simple as possible.
...
To register as a node operator with unlocked FLOW, the token holder can use the **Register Node** ([SC.11](/core-contracts/flow-id-table-staking/#staking))
...
To stake rewarded tokens, the token holder can use the **Re-stake Rewarded FLOW** ([SC.14](/core-contracts/flow-id-table-staking/#staking)) 
transaction with the following arguments:
