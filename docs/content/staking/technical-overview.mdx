...
4. **Rewards Calculation:** Calculate rewards for all the node operators staked in the current epoch.
5. **Remove Insufficiently Staked Nodes:** All node operators who don't meet the minimum
...
object stored in the `FlowEpoch` smart contract.
...
<Callout type="warning">
  ...
  multiple nodes. Multiple nodes need to be registered separately.
</Callout>
