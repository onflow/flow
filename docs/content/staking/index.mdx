...
To register their node, the node operator submits a transaction to register their node along with the tokens they want to stake.
...
with one difference in that 8% of the calculated amount is given to the delegated. 
