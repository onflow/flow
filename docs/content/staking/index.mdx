...
Follow this [step-by-step guide](/) to stake using the Flow Port. (Note: delegating using the Flow Port is not yet available).
Flow Port currently supports staking, delegation and reward withdrawl using **Blocto** and **Ledger** accounts / wallets.
...
custodial account are "locked" by the leagal requirements of the token sale, and _cannot be allocated by their owner_ until the lock period has ended.
...
All supported custody providers should support holding leased tokens you can use to stake your node. We also provide [guides for custody providers](/) on how to set up user accounts to facilitate the use of leased tokens for staking.
