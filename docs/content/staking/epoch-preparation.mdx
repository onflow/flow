...
The number of views in an epoch and in the various epoch phases are known before 
...
so the view number will not change at the same rate as the block height.
...
All voluntary commitments to increase or decrease stake for the next epoch
