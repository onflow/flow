...
If a user is not using the staking collection, they can use the **Create QC Voter** ([QC.01](/core-contracts/epoch))
transaction. This will only store the QC Voter object in the account that stores the `NodeStaker` object.
It does not create a machine account or store it elsewhere.

### Submit Vote

During the Epoch Setup Phase, the node software should submit the votes for the QC generation
automatically using the **Submit QC Vote** ([QC.02](/core-contracts/epoch))
...
If a user is not using the staking collection, they can use the **Create DKG Participant** ([DKG.01](/core-contracts/epoch))
...
automatically using the **Post Whiteboard Message** ([DKG.02](/core-contracts/epoch))
transaction with the following arguments.
...
automatically using the **Send Final Submission** ([DKG.03](/core-contracts/epoch))
transaction with the following arguments.
