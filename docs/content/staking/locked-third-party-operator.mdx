...
1. The node operator [bootstraps a new node record](/node-operation/node-bootstrap/) 
...
This can be done in one of two ways:
  ...
  b) The token holder registers the node using the Flow Port (coming soon).
...
# Option 2: Indirect Staking via a Staking Proxy
...
([NO.01](/protocol/core-contracts/locked-tokens/#node-operator)) transaction.
...
([NO.02](/protocol/core-contracts/locked-tokens/#node-operator)) transaction.
...
These values can be passed into the **Get Operator Node Info** ([TH.15](/protocol/core-contracts/locked-tokens/#token-holder)) 
...
node with the central staking contract by using the using the **Register Node** ([TH.16](/protocol/core-contracts/locked-tokens/#token-holder)) transaction
...
the node operator can use the **Stake New Locked FLOW** ([NO.05](/protocol/core-contracts/locked-tokens/#node-operator)) 
...
the node operator can use the **Re-stake Unstaked FLOW** ([NO.06](/protocol/core-contracts/locked-tokens/#node-operator)) 
transaction with the following arguments:
...
the node operator can use the **Unstake FLOW** ([NO.07](/protocol/core-contracts/locked-tokens/#node-operator)) 
transaction with the following arguments:
...
the node operator can use the **Withdraw Unstaked FLOW** ([NO.08](/protocol/core-contracts/locked-tokens/#node-operator)) 
transaction with the following arguments:
...
the node operator can use the **Withdraw Rewarded FLOW** ([NO.09](/protocol/core-contracts/locked-tokens/#node-operator)) 
transaction with the following arguments:
...
This is done using the **Remove Node Info** ([NO.03](/protocol/core-contracts/locked-tokens/#node-operator)) 
transaction with the following arguments:
