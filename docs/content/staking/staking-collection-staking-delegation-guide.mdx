...
The owner of a Staking Collection can use the **Unstake Tokens** ([SL.08](/core-contracts/staking-collection))
transaction with the following arguments:
...
The owner of a Staking Collection can use the **Unstake Tokens** ([SL.09](/core-contracts/staking-collection))
transaction with the following arguments:
...
The owner of a Staking Collection can use the **Close Stakes** ([SL.10](/core-contracts/staking-collection))
transaction with the following arguments:
...
The owner of a Staking Collection can use the **Close Stakes** ([SL.19](/core-contracts/staking-collection))
transaction with the following arguments:
...
To return an array of structs representing the information associated with each delegation managed by an account's Staking Collection, anyone
...
representing the delegations managed by an accounts Staking Collection.
...
representing the delegator Ids of the each delegation managed by an accounts Staking Collection.
...
To query if a Stake or Delegation is managed by an accounts Staking Collection, anyone
can use the **Get Does Stake Exist** ([SL.15](/core-contracts/staking-collection)) script with the following arguments:

| Argument                | Type               | Description |
|-------------------------|--------------------|-------------|
| **address**             | `Addresss`         | The Address of the account holding the Staking Collection to query from |
| **nodeID**              | `String`           | The nodeID of the node to check, or the nodeID of the node delegating to to check. |
| **delegatorID**         | `Optional(UInt32)` | The delegatorID of the delegator to check, if checking for a delegation. |

<Callout type="info">
To query if a Stake is managed by an accounts Staking Collection, leave the <b>delegatorID</b> arguement as <b>nil</b>. 
Otherwise, fill it in with the <b>delegatorID</b> of the Delegation.
</Callout>
