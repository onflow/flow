...
passing an optional DelegatorID. For example, if you wish to Stake New Tokens for an active Stake,
...
|**`SC.06`**| Stake Rewarded Tokens                 | [stakingCollection/stake_rewarded_tokens.cdc](https://github.com/onflow/flow-core-contracts/blob/master/transactions/stakingCollection/stake_rewarded_tokens.cdc) |
...
The setup process finds any stake or delegation records already stored in an account's storage, as well as any in the associated locked account. It connects them with the new Staking Collection, allowing them
