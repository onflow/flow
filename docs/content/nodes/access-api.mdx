...
The community hosts access node endpoints which are free for everyone to use in the ecosystem.

If you would like to run your own access node nodes please use the application [here](https://flow.com/node-validators).

Alternately, you can also run an [Observer node](../node-operation/observer-node.md) which also provides this gRPC Access API
...
Access nodes operated by Dapper Labs are [rate limited](./access-api-rate-limits.md).
...
The detailed semantics of block formation are covered in the [block formation guide](../concepts/transaction-lifecycle.md#block-formation).
