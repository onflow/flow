...
Please note: The rate limits are applied by each of the access node behind a load balancer and limits listed here are the aggregate total of all the access nodes. Actual limits may be somewhat less depending on how the requests are routed.