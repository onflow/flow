...
The different types of nodes are described [here](/nodes/nodes/node-operation/node-roles). As node operator, you can choose to run any of the different types of node that best fits your needs.
...
To run an observer node, follow this [guide](/nodes/nodes/node-operation/observer-node).
...
Alternately, instead of running your access node, you can use the [Flow community]() access nodes or the ones run by any of the other node operators.
...
1. [Provision](/nodes/nodes/node-operation/node-setup) the machine on which your node will run.

2. [Generate and register](/nodes/nodes/node-operation/node-bootstrap) your node identity.

3. [Start](/nodes/nodes/node-operation/node-bootstrap#step-3---start-your-flow-node) your node!
