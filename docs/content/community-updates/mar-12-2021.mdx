...
We've completed a major overhaul of the Kitty Items marketplace to make it much faster to go from git clone to working app!
Kitty items is still a full-featured NFT marketplace, packed with Flow learning, which demonstrates now more than ever how accessible it is to start building smart contract applications in Flow.

Some highlights include: <br/>
- Dramatically simplified environment configuration
- Simplified account and resource bootstrapping
- Refactored folder names to be more convential
- Refactored API code, consolidating all configuration
- Added a simple 'oracle' pattern; storing sale offers in Postgres via events emitted from Flow

We're excited to see these changes help *even more* people start buiding on Flow.
...
The day has finally arrived! Developers can create a Testnet Flow account without waiting for the Flow team to approve them! The new testnet faucet is here: []()
Simply generate your account keys and complete the captcha and you're ready to start!
