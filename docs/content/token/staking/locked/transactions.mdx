...
The LockedTokens contract defines some events that can be monitored for important actions.

```cadence
pub event SharedAccountRegistered(address: Address)
```
Emitted when a new shared (locked) account is registerd.

```cadence
pub event UnlockedAccountRegistered(address: Address)
```
Emitted when a new user (unlocked) account is registered. This event and `SharedAccountRegistered`
...
```
Emitted when a user registeres their locked account as a node operator.
