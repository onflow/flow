...
These values can be passed into the **Get Operator Node Info** ([TH.04](../transactions/#staking-with-lockboxnodestakerproxy)) 
...
This script returns the `StakingProxy.NodeInfo` struct containing the details for the node. 
...
To register the node as the token holder account's staker, the token holder must use transaction [../transactions/#staking-with-lockboxnodestakerproxy](TH.05):
