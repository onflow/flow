...
![Transaction Anatomy](/assets/transaction-anatomy.png)
...
A transaction that declares different accounts for each signing role will may require more than one signature per account if those accounts use weighted keys to achieve multi-sig functionality.
