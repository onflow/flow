...
This section contains information for individuals, wallet providers, custody providers
and node operators about the FLOW token, including how it can be acquired, 
transferred and used for common actions such as staking.

### FLOW as a native token

FLOW is the default token for the Flow protocol, which means it is used for all protocol-level fee payments, 
...
There are three general ways to acquire new FLOW tokens:

1. [**FLOW for Investors**](#flow-for-investors): Purchase as an investor through an initial token sale.
2. [**FLOW for Validators**](): Receive newly-minted FLOW as a staking reward.
3. [**FLOW for Consumers**](): Purchase from an external exchange that supports FLOW (TBA).
