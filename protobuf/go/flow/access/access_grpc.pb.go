...
type AccessAPIClient interface {
	...
	// GetScheduledTransaction gets a scheduled transaction body for a given
	// callback ID
	...
	// GetScheduledTransactionResult gets a scheduled transaction result for a
	// given callback ID
	...
	// GetAccountBalanceAtLatestBlock gets an account balance by address from the
	...
	// GetAccountBalanceAtBlockHeight gets an account balance by address at the
	// given block height
	...
	// GetAccountKeysAtLatestBlock gets an account public keys by address from the
	// latest sealed execution state.
	...
	// GetAccountKeysAtBlockHeight gets an account public keys by address at the
	// given block height
	...
	// GetAccountKeysAtLatestBlock gets an account public key by address and key
	// index from the latest sealed execution state.
	...
	// GetAccountKeysAtBlockHeight gets an account public key by address and key
	// index at the given block height
	...
	// GetProtocolStateSnapshotByBlockID retrieves the latest sealed protocol
	// state snapshot by block ID. Used by Flow nodes joining the network to
	// bootstrap a space-efficient local state.
	...
	// snapshot by block height. Used by Flow nodes joining the network to
	// bootstrap a space-efficient local state.
	...
	//
	// Blocks are only returned when they have reached the provided block status.
	// For example, if the status is "sealed", only sealed blocks will be
	// returned.
	SubscribeBlocksFromStartBlockID(ctx context.Context, in *SubscribeBlocksFromStartBlockIDRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlocksResponse], error)
	// SubscribeBlocksFromStartHeight streams finalized or sealed blocks starting
	// at the requested start block height, up until the latest available block.
	// Once the latest is reached, the stream will remain open and responses are
	// sent for each new block as it becomes available.
	//
	// Blocks are only returned when they have reached the provided block status.
	// For example, if the status is "sealed", only sealed blocks will be
	// returned.
	SubscribeBlocksFromStartHeight(ctx context.Context, in *SubscribeBlocksFromStartHeightRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlocksResponse], error)
	// SubscribeBlocksFromLatest streams finalized or sealed blocks starting from
	// the latest finalized or sealed block. The stream will remain open and
	// responses are sent for each new block as it becomes available.
	//
	// Blocks are only returned when they have reached the provided block status.
	// For example, if the status is "sealed", only sealed blocks will be
	// returned.
	SubscribeBlocksFromLatest(ctx context.Context, in *SubscribeBlocksFromLatestRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlocksResponse], error)
	// SubscribeBlockHeadersFromStartBlockID streams finalized or sealed block
	// headers starting at the requested start block id, up until the latest
	// available block. Once the latest is reached, the stream will remain open
	// and responses are sent for each new block header as it becomes available.
	//
	// Block headers are only returned when they have reached the provided block
	// status. For example, if the status is "sealed", only sealed block headers
	// will be returned.
	SubscribeBlockHeadersFromStartBlockID(ctx context.Context, in *SubscribeBlockHeadersFromStartBlockIDRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockHeadersResponse], error)
	// SubscribeBlockHeadersFromStartHeight streams finalized or sealed block
	// headers starting at the requested start block height, up until the latest
	// available block. Once the latest is reached, the stream will remain open
	// and responses are sent for each new block header as it becomes available.
	//
	// Block headers are only returned when they have reached the provided block
	// status. For example, if the status is "sealed", only sealed block headers
	// will be returned.
	SubscribeBlockHeadersFromStartHeight(ctx context.Context, in *SubscribeBlockHeadersFromStartHeightRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockHeadersResponse], error)
	// SubscribeBlockHeadersFromLatest streams finalized or sealed block headers
	// starting from the latest finalized or sealed block. The stream will remain
	// open and responses are sent for each new block header as it becomes
	// available.
	//
	// Block headers are only returned when they have reached the provided block
	// status. For example, if the status is "sealed", only sealed block headers
	// will be returned.
	SubscribeBlockHeadersFromLatest(ctx context.Context, in *SubscribeBlockHeadersFromLatestRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockHeadersResponse], error)
	// SubscribeBlockDigestsFromStartBlockID streams finalized or sealed
	// lightweight block starting at the requested start block id, up until the
	// latest available block. Once the latest is reached, the stream will remain
	// open and responses are sent for each new lightweight block as it becomes
	// available.
	//
	// Lightweight blocks are only returned when they have reached the provided
	// block status. For example, if the status is "sealed", only sealed
	// lightweight blocks will be returned.
	SubscribeBlockDigestsFromStartBlockID(ctx context.Context, in *SubscribeBlockDigestsFromStartBlockIDRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockDigestsResponse], error)
	// SubscribeBlockDigestsFromStartHeight streams finalized or sealed
	// lightweight block starting at the requested start block height, up until
	// the latest available block. Once the latest is reached, the stream will
	// remain open and responses are sent for each new lightweight block as it
	// becomes available.
	//
	// Lightweight blocks are only returned when they have reached the provided
	// block status. For example, if the status is "sealed", only sealed
	// lightweight blocks will be returned.
	SubscribeBlockDigestsFromStartHeight(ctx context.Context, in *SubscribeBlockDigestsFromStartHeightRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockDigestsResponse], error)
	// SubscribeBlockDigestsFromLatest streams finalized or sealed lightweight
	// block headers starting of the latest finalized or sealed block. The stream
	// will remain open and responses are sent for each new lightweight block as
	// it becomes available.
	//
	// Lightweight blocks are only returned when they have reached the provided
	// block status. For example, if the status is "sealed", only sealed
	// lightweight blocks will be returned.
	SubscribeBlockDigestsFromLatest(ctx context.Context, in *SubscribeBlockDigestsFromLatestRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockDigestsResponse], error)
	// SendAndSubscribeTransactionStatuses send a transaction and immediately
	// subscribe to its status changes. The status is streamed back until the
	// block containing the transaction becomes sealed.
	SendAndSubscribeTransactionStatuses(ctx context.Context, in *SendAndSubscribeTransactionStatusesRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SendAndSubscribeTransactionStatusesResponse], error)
}
...
func (c *accessAPIClient) Ping(ctx context.Context, in *PingRequest, opts ...grpc.CallOption) (*PingResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_Ping_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetNodeVersionInfo(ctx context.Context, in *GetNodeVersionInfoRequest, opts ...grpc.CallOption) (*GetNodeVersionInfoResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetNodeVersionInfo_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetLatestBlockHeader(ctx context.Context, in *GetLatestBlockHeaderRequest, opts ...grpc.CallOption) (*BlockHeaderResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetLatestBlockHeader_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetBlockHeaderByID(ctx context.Context, in *GetBlockHeaderByIDRequest, opts ...grpc.CallOption) (*BlockHeaderResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(BlockHeaderResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetBlockHeaderByID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetBlockHeaderByHeight(ctx context.Context, in *GetBlockHeaderByHeightRequest, opts ...grpc.CallOption) (*BlockHeaderResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(BlockHeaderResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetBlockHeaderByHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetLatestBlock(ctx context.Context, in *GetLatestBlockRequest, opts ...grpc.CallOption) (*BlockResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetLatestBlock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetBlockByID(ctx context.Context, in *GetBlockByIDRequest, opts ...grpc.CallOption) (*BlockResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(BlockResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetBlockByID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetBlockByHeight(ctx context.Context, in *GetBlockByHeightRequest, opts ...grpc.CallOption) (*BlockResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(BlockResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetBlockByHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetCollectionByID(ctx context.Context, in *GetCollectionByIDRequest, opts ...grpc.CallOption) (*CollectionResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetCollectionByID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetFullCollectionByID(ctx context.Context, in *GetFullCollectionByIDRequest, opts ...grpc.CallOption) (*FullCollectionResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetFullCollectionByID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) SendTransaction(ctx context.Context, in *SendTransactionRequest, opts ...grpc.CallOption) (*SendTransactionResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_SendTransaction_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetTransaction(ctx context.Context, in *GetTransactionRequest, opts ...grpc.CallOption) (*TransactionResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetTransaction_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetTransactionResult(ctx context.Context, in *GetTransactionRequest, opts ...grpc.CallOption) (*TransactionResultResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetTransactionResult_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetTransactionResultByIndex(ctx context.Context, in *GetTransactionByIndexRequest, opts ...grpc.CallOption) (*TransactionResultResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(TransactionResultResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetTransactionResultByIndex_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetTransactionResultsByBlockID(ctx context.Context, in *GetTransactionsByBlockIDRequest, opts ...grpc.CallOption) (*TransactionResultsResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetTransactionResultsByBlockID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetTransactionsByBlockID(ctx context.Context, in *GetTransactionsByBlockIDRequest, opts ...grpc.CallOption) (*TransactionsResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetTransactionsByBlockID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetSystemTransaction(ctx context.Context, in *GetSystemTransactionRequest, opts ...grpc.CallOption) (*TransactionResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(TransactionResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetSystemTransaction_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetSystemTransactionResult(ctx context.Context, in *GetSystemTransactionResultRequest, opts ...grpc.CallOption) (*TransactionResultResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(TransactionResultResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetSystemTransactionResult_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetScheduledTransaction(ctx context.Context, in *GetScheduledTransactionRequest, opts ...grpc.CallOption) (*TransactionResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(TransactionResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetScheduledTransaction_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetScheduledTransactionResult(ctx context.Context, in *GetScheduledTransactionResultRequest, opts ...grpc.CallOption) (*TransactionResultResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(TransactionResultResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetScheduledTransactionResult_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccount(ctx context.Context, in *GetAccountRequest, opts ...grpc.CallOption) (*GetAccountResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetAccount_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountAtLatestBlock(ctx context.Context, in *GetAccountAtLatestBlockRequest, opts ...grpc.CallOption) (*AccountResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountAtLatestBlock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountAtBlockHeight(ctx context.Context, in *GetAccountAtBlockHeightRequest, opts ...grpc.CallOption) (*AccountResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AccountResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountAtBlockHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountBalanceAtLatestBlock(ctx context.Context, in *GetAccountBalanceAtLatestBlockRequest, opts ...grpc.CallOption) (*AccountBalanceResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountBalanceAtLatestBlock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountBalanceAtBlockHeight(ctx context.Context, in *GetAccountBalanceAtBlockHeightRequest, opts ...grpc.CallOption) (*AccountBalanceResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AccountBalanceResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountBalanceAtBlockHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountKeysAtLatestBlock(ctx context.Context, in *GetAccountKeysAtLatestBlockRequest, opts ...grpc.CallOption) (*AccountKeysResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountKeysAtLatestBlock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountKeysAtBlockHeight(ctx context.Context, in *GetAccountKeysAtBlockHeightRequest, opts ...grpc.CallOption) (*AccountKeysResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AccountKeysResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountKeysAtBlockHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountKeyAtLatestBlock(ctx context.Context, in *GetAccountKeyAtLatestBlockRequest, opts ...grpc.CallOption) (*AccountKeyResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountKeyAtLatestBlock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetAccountKeyAtBlockHeight(ctx context.Context, in *GetAccountKeyAtBlockHeightRequest, opts ...grpc.CallOption) (*AccountKeyResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AccountKeyResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetAccountKeyAtBlockHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) ExecuteScriptAtLatestBlock(ctx context.Context, in *ExecuteScriptAtLatestBlockRequest, opts ...grpc.CallOption) (*ExecuteScriptResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_ExecuteScriptAtLatestBlock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) ExecuteScriptAtBlockID(ctx context.Context, in *ExecuteScriptAtBlockIDRequest, opts ...grpc.CallOption) (*ExecuteScriptResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ExecuteScriptResponse)
	err := c.cc.Invoke(ctx, AccessAPI_ExecuteScriptAtBlockID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) ExecuteScriptAtBlockHeight(ctx context.Context, in *ExecuteScriptAtBlockHeightRequest, opts ...grpc.CallOption) (*ExecuteScriptResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ExecuteScriptResponse)
	err := c.cc.Invoke(ctx, AccessAPI_ExecuteScriptAtBlockHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetEventsForHeightRange(ctx context.Context, in *GetEventsForHeightRangeRequest, opts ...grpc.CallOption) (*EventsResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetEventsForHeightRange_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetEventsForBlockIDs(ctx context.Context, in *GetEventsForBlockIDsRequest, opts ...grpc.CallOption) (*EventsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(EventsResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetEventsForBlockIDs_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetNetworkParameters(ctx context.Context, in *GetNetworkParametersRequest, opts ...grpc.CallOption) (*GetNetworkParametersResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetNetworkParameters_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetLatestProtocolStateSnapshot(ctx context.Context, in *GetLatestProtocolStateSnapshotRequest, opts ...grpc.CallOption) (*ProtocolStateSnapshotResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetLatestProtocolStateSnapshot_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetProtocolStateSnapshotByBlockID(ctx context.Context, in *GetProtocolStateSnapshotByBlockIDRequest, opts ...grpc.CallOption) (*ProtocolStateSnapshotResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ProtocolStateSnapshotResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetProtocolStateSnapshotByBlockID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetProtocolStateSnapshotByHeight(ctx context.Context, in *GetProtocolStateSnapshotByHeightRequest, opts ...grpc.CallOption) (*ProtocolStateSnapshotResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ProtocolStateSnapshotResponse)
	err := c.cc.Invoke(ctx, AccessAPI_GetProtocolStateSnapshotByHeight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetExecutionResultForBlockID(ctx context.Context, in *GetExecutionResultForBlockIDRequest, opts ...grpc.CallOption) (*ExecutionResultForBlockIDResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetExecutionResultForBlockID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) GetExecutionResultByID(ctx context.Context, in *GetExecutionResultByIDRequest, opts ...grpc.CallOption) (*ExecutionResultByIDResponse, error) {
	...
	err := c.cc.Invoke(ctx, AccessAPI_GetExecutionResultByID_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accessAPIClient) SubscribeBlocksFromStartBlockID(ctx context.Context, in *SubscribeBlocksFromStartBlockIDRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlocksResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[0], AccessAPI_SubscribeBlocksFromStartBlockID_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlocksFromStartBlockIDRequest, SubscribeBlocksResponse]{ClientStream: stream}
...
type AccessAPI_SubscribeBlocksFromStartBlockIDClient = grpc.ServerStreamingClient[SubscribeBlocksResponse]

func (c *accessAPIClient) SubscribeBlocksFromStartHeight(ctx context.Context, in *SubscribeBlocksFromStartHeightRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlocksResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[1], AccessAPI_SubscribeBlocksFromStartHeight_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlocksFromStartHeightRequest, SubscribeBlocksResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlocksFromStartHeightClient = grpc.ServerStreamingClient[SubscribeBlocksResponse]

func (c *accessAPIClient) SubscribeBlocksFromLatest(ctx context.Context, in *SubscribeBlocksFromLatestRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlocksResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[2], AccessAPI_SubscribeBlocksFromLatest_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlocksFromLatestRequest, SubscribeBlocksResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlocksFromLatestClient = grpc.ServerStreamingClient[SubscribeBlocksResponse]

func (c *accessAPIClient) SubscribeBlockHeadersFromStartBlockID(ctx context.Context, in *SubscribeBlockHeadersFromStartBlockIDRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockHeadersResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[3], AccessAPI_SubscribeBlockHeadersFromStartBlockID_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlockHeadersFromStartBlockIDRequest, SubscribeBlockHeadersResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockHeadersFromStartBlockIDClient = grpc.ServerStreamingClient[SubscribeBlockHeadersResponse]

func (c *accessAPIClient) SubscribeBlockHeadersFromStartHeight(ctx context.Context, in *SubscribeBlockHeadersFromStartHeightRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockHeadersResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[4], AccessAPI_SubscribeBlockHeadersFromStartHeight_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlockHeadersFromStartHeightRequest, SubscribeBlockHeadersResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockHeadersFromStartHeightClient = grpc.ServerStreamingClient[SubscribeBlockHeadersResponse]

func (c *accessAPIClient) SubscribeBlockHeadersFromLatest(ctx context.Context, in *SubscribeBlockHeadersFromLatestRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockHeadersResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[5], AccessAPI_SubscribeBlockHeadersFromLatest_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlockHeadersFromLatestRequest, SubscribeBlockHeadersResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockHeadersFromLatestClient = grpc.ServerStreamingClient[SubscribeBlockHeadersResponse]

func (c *accessAPIClient) SubscribeBlockDigestsFromStartBlockID(ctx context.Context, in *SubscribeBlockDigestsFromStartBlockIDRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockDigestsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[6], AccessAPI_SubscribeBlockDigestsFromStartBlockID_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlockDigestsFromStartBlockIDRequest, SubscribeBlockDigestsResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockDigestsFromStartBlockIDClient = grpc.ServerStreamingClient[SubscribeBlockDigestsResponse]

func (c *accessAPIClient) SubscribeBlockDigestsFromStartHeight(ctx context.Context, in *SubscribeBlockDigestsFromStartHeightRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockDigestsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[7], AccessAPI_SubscribeBlockDigestsFromStartHeight_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlockDigestsFromStartHeightRequest, SubscribeBlockDigestsResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockDigestsFromStartHeightClient = grpc.ServerStreamingClient[SubscribeBlockDigestsResponse]

func (c *accessAPIClient) SubscribeBlockDigestsFromLatest(ctx context.Context, in *SubscribeBlockDigestsFromLatestRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SubscribeBlockDigestsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[8], AccessAPI_SubscribeBlockDigestsFromLatest_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscribeBlockDigestsFromLatestRequest, SubscribeBlockDigestsResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockDigestsFromLatestClient = grpc.ServerStreamingClient[SubscribeBlockDigestsResponse]

func (c *accessAPIClient) SendAndSubscribeTransactionStatuses(ctx context.Context, in *SendAndSubscribeTransactionStatusesRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SendAndSubscribeTransactionStatusesResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AccessAPI_ServiceDesc.Streams[9], AccessAPI_SendAndSubscribeTransactionStatuses_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SendAndSubscribeTransactionStatusesRequest, SendAndSubscribeTransactionStatusesResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SendAndSubscribeTransactionStatusesClient = grpc.ServerStreamingClient[SendAndSubscribeTransactionStatusesResponse]
...
type AccessAPIServer interface {
	...
	// For example, if the status is "sealed", only sealed blocks will be
	// returned.
	SubscribeBlocksFromStartBlockID(*SubscribeBlocksFromStartBlockIDRequest, grpc.ServerStreamingServer[SubscribeBlocksResponse]) error
	// SubscribeBlocksFromStartHeight streams finalized or sealed blocks starting
	// at the requested start block height, up until the latest available block.
	// Once the latest is reached, the stream will remain open and responses are
	// sent for each new block as it becomes available.
	...
	// For example, if the status is "sealed", only sealed blocks will be
	// returned.
	SubscribeBlocksFromStartHeight(*SubscribeBlocksFromStartHeightRequest, grpc.ServerStreamingServer[SubscribeBlocksResponse]) error
	// SubscribeBlocksFromLatest streams finalized or sealed blocks starting from
	// the latest finalized or sealed block. The stream will remain open and
	// responses are sent for each new block as it becomes available.
	//
	...
	// For example, if the status is "sealed", only sealed blocks will be
	// returned.
	SubscribeBlocksFromLatest(*SubscribeBlocksFromLatestRequest, grpc.ServerStreamingServer[SubscribeBlocksResponse]) error
	// SubscribeBlockHeadersFromStartBlockID streams finalized or sealed block
	// headers starting at the requested start block id, up until the latest
	// available block. Once the latest is reached, the stream will remain open
	// and responses are sent for each new block header as it becomes available.
	...
	// status. For example, if the status is "sealed", only sealed block headers
	// will be returned.
	SubscribeBlockHeadersFromStartBlockID(*SubscribeBlockHeadersFromStartBlockIDRequest, grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]) error
	// SubscribeBlockHeadersFromStartHeight streams finalized or sealed block
	// headers starting at the requested start block height, up until the latest
	// available block. Once the latest is reached, the stream will remain open
	// and responses are sent for each new block header as it becomes available.
	...
	// status. For example, if the status is "sealed", only sealed block headers
	// will be returned.
	SubscribeBlockHeadersFromStartHeight(*SubscribeBlockHeadersFromStartHeightRequest, grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]) error
	// SubscribeBlockHeadersFromLatest streams finalized or sealed block headers
	// starting from the latest finalized or sealed block. The stream will remain
	// open and responses are sent for each new block header as it becomes
	// available.
	...
	// status. For example, if the status is "sealed", only sealed block headers
	// will be returned.
	SubscribeBlockHeadersFromLatest(*SubscribeBlockHeadersFromLatestRequest, grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]) error
	// SubscribeBlockDigestsFromStartBlockID streams finalized or sealed
	// lightweight block starting at the requested start block id, up until the
	// latest available block. Once the latest is reached, the stream will remain
	// open and responses are sent for each new lightweight block as it becomes
	...
	// block status. For example, if the status is "sealed", only sealed
	// lightweight blocks will be returned.
	SubscribeBlockDigestsFromStartBlockID(*SubscribeBlockDigestsFromStartBlockIDRequest, grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]) error
	// SubscribeBlockDigestsFromStartHeight streams finalized or sealed
	// lightweight block starting at the requested start block height, up until
	// the latest available block. Once the latest is reached, the stream will
	// remain open and responses are sent for each new lightweight block as it
	...
	// block status. For example, if the status is "sealed", only sealed
	// lightweight blocks will be returned.
	SubscribeBlockDigestsFromStartHeight(*SubscribeBlockDigestsFromStartHeightRequest, grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]) error
	// SubscribeBlockDigestsFromLatest streams finalized or sealed lightweight
	// block headers starting of the latest finalized or sealed block. The stream
	// will remain open and responses are sent for each new lightweight block as
	// it becomes available.
	...
	// block status. For example, if the status is "sealed", only sealed
	// lightweight blocks will be returned.
	SubscribeBlockDigestsFromLatest(*SubscribeBlockDigestsFromLatestRequest, grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]) error
	// SendAndSubscribeTransactionStatuses send a transaction and immediately
	// subscribe to its status changes. The status is streamed back until the
	// block containing the transaction becomes sealed.
	SendAndSubscribeTransactionStatuses(*SendAndSubscribeTransactionStatusesRequest, grpc.ServerStreamingServer[SendAndSubscribeTransactionStatusesResponse]) error
}

// UnimplementedAccessAPIServer should be embedded to have
...
func (UnimplementedAccessAPIServer) GetLatestBlockHeader(context.Context, *GetLatestBlockHeaderRequest) (*BlockHeaderResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetLatestBlockHeader not implemented")
}
func (UnimplementedAccessAPIServer) GetBlockHeaderByID(context.Context, *GetBlockHeaderByIDRequest) (*BlockHeaderResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetBlockHeaderByID not implemented")
}
func (UnimplementedAccessAPIServer) GetBlockHeaderByHeight(context.Context, *GetBlockHeaderByHeightRequest) (*BlockHeaderResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetBlockHeaderByHeight not implemented")
}
...
func (UnimplementedAccessAPIServer) GetAccountBalanceAtLatestBlock(context.Context, *GetAccountBalanceAtLatestBlockRequest) (*AccountBalanceResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetAccountBalanceAtLatestBlock not implemented")
}
func (UnimplementedAccessAPIServer) GetAccountBalanceAtBlockHeight(context.Context, *GetAccountBalanceAtBlockHeightRequest) (*AccountBalanceResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetAccountBalanceAtBlockHeight not implemented")
}
...
func (UnimplementedAccessAPIServer) GetNetworkParameters(context.Context, *GetNetworkParametersRequest) (*GetNetworkParametersResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetNetworkParameters not implemented")
}
...
func (UnimplementedAccessAPIServer) SubscribeBlocksFromStartBlockID(*SubscribeBlocksFromStartBlockIDRequest, grpc.ServerStreamingServer[SubscribeBlocksResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlocksFromStartBlockID not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlocksFromStartHeight(*SubscribeBlocksFromStartHeightRequest, grpc.ServerStreamingServer[SubscribeBlocksResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlocksFromStartHeight not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlocksFromLatest(*SubscribeBlocksFromLatestRequest, grpc.ServerStreamingServer[SubscribeBlocksResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlocksFromLatest not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlockHeadersFromStartBlockID(*SubscribeBlockHeadersFromStartBlockIDRequest, grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlockHeadersFromStartBlockID not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlockHeadersFromStartHeight(*SubscribeBlockHeadersFromStartHeightRequest, grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlockHeadersFromStartHeight not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlockHeadersFromLatest(*SubscribeBlockHeadersFromLatestRequest, grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlockHeadersFromLatest not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlockDigestsFromStartBlockID(*SubscribeBlockDigestsFromStartBlockIDRequest, grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlockDigestsFromStartBlockID not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlockDigestsFromStartHeight(*SubscribeBlockDigestsFromStartHeightRequest, grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlockDigestsFromStartHeight not implemented")
}
func (UnimplementedAccessAPIServer) SubscribeBlockDigestsFromLatest(*SubscribeBlockDigestsFromLatestRequest, grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]) error {
	return status.Error(codes.Unimplemented, "method SubscribeBlockDigestsFromLatest not implemented")
}
func (UnimplementedAccessAPIServer) SendAndSubscribeTransactionStatuses(*SendAndSubscribeTransactionStatusesRequest, grpc.ServerStreamingServer[SendAndSubscribeTransactionStatusesResponse]) error {
	return status.Error(codes.Unimplemented, "method SendAndSubscribeTransactionStatuses not implemented")
}
...
func RegisterAccessAPIServer(s grpc.ServiceRegistrar, srv AccessAPIServer) {
	// If the following call panics, it indicates UnimplementedAccessAPIServer was
...
func _AccessAPI_SubscribeBlocksFromStartBlockID_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlocksFromStartBlockID(m, &grpc.GenericServerStream[SubscribeBlocksFromStartBlockIDRequest, SubscribeBlocksResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlocksFromStartBlockIDServer = grpc.ServerStreamingServer[SubscribeBlocksResponse]
...
func _AccessAPI_SubscribeBlocksFromStartHeight_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlocksFromStartHeight(m, &grpc.GenericServerStream[SubscribeBlocksFromStartHeightRequest, SubscribeBlocksResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlocksFromStartHeightServer = grpc.ServerStreamingServer[SubscribeBlocksResponse]
...
func _AccessAPI_SubscribeBlocksFromLatest_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlocksFromLatest(m, &grpc.GenericServerStream[SubscribeBlocksFromLatestRequest, SubscribeBlocksResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlocksFromLatestServer = grpc.ServerStreamingServer[SubscribeBlocksResponse]
...
func _AccessAPI_SubscribeBlockHeadersFromStartBlockID_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlockHeadersFromStartBlockID(m, &grpc.GenericServerStream[SubscribeBlockHeadersFromStartBlockIDRequest, SubscribeBlockHeadersResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockHeadersFromStartBlockIDServer = grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]
...
func _AccessAPI_SubscribeBlockHeadersFromStartHeight_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlockHeadersFromStartHeight(m, &grpc.GenericServerStream[SubscribeBlockHeadersFromStartHeightRequest, SubscribeBlockHeadersResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockHeadersFromStartHeightServer = grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]
...
func _AccessAPI_SubscribeBlockHeadersFromLatest_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlockHeadersFromLatest(m, &grpc.GenericServerStream[SubscribeBlockHeadersFromLatestRequest, SubscribeBlockHeadersResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockHeadersFromLatestServer = grpc.ServerStreamingServer[SubscribeBlockHeadersResponse]
...
func _AccessAPI_SubscribeBlockDigestsFromStartBlockID_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlockDigestsFromStartBlockID(m, &grpc.GenericServerStream[SubscribeBlockDigestsFromStartBlockIDRequest, SubscribeBlockDigestsResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockDigestsFromStartBlockIDServer = grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]
...
func _AccessAPI_SubscribeBlockDigestsFromStartHeight_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlockDigestsFromStartHeight(m, &grpc.GenericServerStream[SubscribeBlockDigestsFromStartHeightRequest, SubscribeBlockDigestsResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockDigestsFromStartHeightServer = grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]
...
func _AccessAPI_SubscribeBlockDigestsFromLatest_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SubscribeBlockDigestsFromLatest(m, &grpc.GenericServerStream[SubscribeBlockDigestsFromLatestRequest, SubscribeBlockDigestsResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SubscribeBlockDigestsFromLatestServer = grpc.ServerStreamingServer[SubscribeBlockDigestsResponse]
...
func _AccessAPI_SendAndSubscribeTransactionStatuses_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AccessAPIServer).SendAndSubscribeTransactionStatuses(m, &grpc.GenericServerStream[SendAndSubscribeTransactionStatusesRequest, SendAndSubscribeTransactionStatusesResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AccessAPI_SendAndSubscribeTransactionStatusesServer = grpc.ServerStreamingServer[SendAndSubscribeTransactionStatusesResponse]
